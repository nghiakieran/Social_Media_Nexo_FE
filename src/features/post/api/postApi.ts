...
export const createPost = async (
  ...
  try {
    ...
    if (error && typeof error === "object" && "response" in error) {
      ...
      throw new Error(
        apiError.response?.data?.message || "Có lỗi xảy ra khi tạo bài viết"
      );
    }
...
export const getFeed = async (
  ...
  try {
    ...
    if (error && typeof error === "object" && "response" in error) {
      const apiError = error as { response?: { data?: { message?: string } } };
      throw new Error(
        apiError.response?.data?.message || "Có lỗi xảy ra khi tải feed"
      );
    }
...
export const togglePostActive = async (
  postId: number
): Promise<TogglePostActiveResponse> => {
  try {
    const response = await api.patch<TogglePostActiveResponse>(
      `/posts/${postId}`
    );
    return response.data;
  } catch (error: unknown) {
...
export const deletePost = async (
  ...
  try {
    ...
    if (error && typeof error === "object" && "response" in error) {
      const apiError = error as { response?: { data?: { message?: string } } };
      throw new Error(
        apiError.response?.data?.message || "Có lỗi xảy ra khi xóa bài viết"
      );
    }
...
export const bookmarkPost = async (
  ...
  try {
    const response = await api.post<BookmarkPostResponse>(
...
export const getMutualFollowers = async (
  ...
  try {
    ...
    const response = await api.get<GetMutualFollowersApiResponse>(
      "/users/mutuals",
      {
        params: apiParams,
      }
...
export const reportPost = async (
  ...
  try {
    const response = await api.post<any>(`/posts/report/post/${postId}`, {
      reason,
      detail,
    });
    return response.data.data;
