...
export const likeComment = async (commentId: number): Promise<LikeResponse> => {
  try {
    const response = await api.post<LikeResponse>(`/interaction/like/comment/${commentId}`);
...
export const likePost = async (postId: number): Promise<LikeResponse> => {
  try {
    const response = await api.post<LikeResponse>(`/interaction/like/post/${postId}`);
    return response.data;
  } catch (error: unknown) {
    if (error && typeof error === "object" && "response" in error) {
      const apiError = error as { response?: { data?: { message?: string } } };
...
export const likeReel = async (reelId: number): Promise<LikeResponse> => {
  try {
    const response = await api.post<LikeResponse>(`/interaction/like/reel/${reelId}`);
    return response.data;
  } catch (error: unknown) {
    if (error && typeof error === "object" && "response" in error) {
      const apiError = error as { response?: { data?: { message?: string } } };
